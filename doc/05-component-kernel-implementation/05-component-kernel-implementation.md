...
> and interpreted (i.e., by correspondence).
   ...
   I will model the Inventory using an Array of Items for the inventory itself and a Set of Strings for the tag restrictions list. An Array will allow the Items to be randomly accessed and will enforce an inventory having a fixed size. A Set will allow tags to be added to the restriction list after the Inventory is initialized and ensure no duplicates exist.

   The Set will contain no duplicate Strings. The Set will not contain the String defined in Item.COUNT. The Array will contain no null Items. The Items in the Array will have all the tags contained in the Set.

   The Item at index i in the Array is the Item at slot i in the Inventory. The Set is the list of tag requirements with no duplicates for an Item to be in the Inventory.
